...
# from torch_scatter import scatter_add,scatter_mean,scatter_max,scatter_min need to change cuda version to 11.0
...
def points_to_voxels(points_xyz,
  ...
  # points_mask_new = points_mask
  batch_size, num_points, _ = points_xyz.shape
  # points_mask_new = torch.reshape(points_mask_new, [batch_size, num_points])
...
def _batched_unsorted_segment_fn(batched_data,
  ...
  # print('batched_segment_ids.dtype',batched_segment_ids.dtype,'batched_segment_ids.device',batched_segment_ids.device,'batch_size is',batch_size)
  ...
  # batched_data = tf.cast(batched_data.cpu().numpy(), dtype=tf.int32)
  # batched_segment_ids = tf.cast(batched_segment_ids.cpu().numpy(), dtype=tf.int32)
  if unsorted_segment_fn == scatter_add:
    ...
    # batched_segment_output = tf.math.segment_max(tf.cast(batched_data.cpu().detach().numpy(), dtype=tf.float32),
    #                                            tf.cast(batched_segment_ids.cpu().detach().numpy(), dtype=tf.int32),
    ...
    zero = batch_size * num_segments-batched_segment_output.shape[0]
    batched_segment_output = torch.nn.functional.pad(batched_segment_output,[0,0,0,zero],value=0)
  # batched_segment_output = torch.Tensor(batched_segment_output.numpy()).type(batched_data.dtype).to(batched_data.device)
