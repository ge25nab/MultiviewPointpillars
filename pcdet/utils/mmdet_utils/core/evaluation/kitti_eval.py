...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    #gt_annos/dt_annos list [3769] 3769 from training folder for validation
    #gt_annos[0].keys()
    #is dict_keys(['name', 'truncated', 'occluded', 'alpha', 'bbox', 'dimensions', 
    #'location', 'rotation_y', 'score', 'difficulty', 'index', 'gt_boxes_lidar', 'num_points_in_gt'])
    ...
    for anno in dt_annos:
        if anno['alpha'].shape[0] != 0:
            if anno['alpha'][0] != -10:
                compute_aos = True
            break
    mAPbbox, mAPbev, mAP3d, mAPaos, mAPbbox_R40, mAPbev_R40, mAP3d_R40, mAPaos_R40 = do_eval_v3(
